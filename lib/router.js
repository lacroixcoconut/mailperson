    ...
    class Router {
        ...
        constructor(config){
            ...
            Object.keys(config).forEach(path => {
                ...
                routePairs.push([path, routeConfig.file.filename])

                this._router.get(path, ...this.showForm(path, routeConfig));
                this._router.post(path, bodyParser(), ...this.sendFile(path, routeConfig));
            });
            
            if(config.index !== false) this._router.get('/', ...this.showIndex(config.index, routePairs));
        }

        showIndex(routeConfig={}, routePairs){
            ...
            middlewares.push(async (ctx, next) => {
                ctx.body = await renderer.index(routePairs);
            });
